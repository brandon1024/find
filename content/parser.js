...
Find.register('Content.Parser', function (self) {
    ...
    self.buildDOMReferenceObject = function () {
        ...
        const preformatted = { flag: false, index: null };
        const hidden = { flag: false, index: null };
        ...
        while (!reachedEndOfTree) {
            ...
            const textGroup = { group: [], preformatted: false };
            while (node) {
                const nodeDepth = getNodeTreeDepth(node);
                ...
                if (isElementNode(node)) {
                    ...
                    const identifierUUID = generateElementUUID();
                    const nodeText = formatTextNodeValue(node, preformatted.flag, elementBoundary);
                    ...
                    const textNodeInformation = { groupIndex: groupIndex, text: nodeText, elementUUID: identifierUUID };
    ...
    function formatTextNodeValue(node, preformatted, elementBoundary) {
        ...
        const nodeText = decode(node.nodeValue);
        if (preformatted) {
            return nodeText;
        }
